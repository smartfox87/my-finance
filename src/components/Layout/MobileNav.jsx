...
export const MobileNav = memo(function MobileNav() {
  ...
  const getNavLinkClassName = (url) => (url === pathname ? "text-blue-600 dark:text-blue-400" : "");
        ...
        {filteredList.map(({ icon, full_name, short_name, url }) => (
          <li key={full_name} className="w-1/12 grow">
            <Link
              ...
              className={`${getNavLinkClassName(getLocalizeUrl(url))} flex flex-col items-center gap-1 py-2 text-center text-xs duration-300 hover:text-blue-600 lg:-ml-3 lg:mr-3 lg:flex-row lg:gap-4 lg:py-4 lg:pl-3 lg:pr-3 lg:text-left lg:text-xl dark:hover:text-blue-400 `}
            >
              {icon}
